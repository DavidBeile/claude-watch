...
- 35 tests in `scripts/tests/test_youtube.py` covering duration parsing, Analytics row shaping, retention summarisation, token-freshness logic, token-file permissions, the bounded OAuth callback wait and short-form pagination. The 15 added with the fixes above were checked against the pre-fix code first and fail there.

### Fixed
- **Refresh token is no longer briefly world-readable.** `_write_token()` wrote the file with `write_text()` — created under the process umask, so 0644 on a default `022` — and only afterwards chmod'ed it to 0600. Any local user could read the credential in that window, and if the chmod failed the token simply kept its old mode with nothing but a warning. It is now created with `O_CREAT|O_EXCL` at mode 0600 and moved into place with `os.replace()`, and a failure to create it securely aborts instead of writing the token anyway. Found by an adversarial Codex review.
- **`auth` can no longer hang forever.** The loopback consent flow called `server.handle_request()` with no timeout, so a browser that never reached 127.0.0.1, a closed consent tab, or a redirect that never arrived left the command blocked with no recovery but killing the process. The wait is now bounded (`OAUTH_CALLBACK_TIMEOUT`, 180s) and runs as a deadline loop that ignores unrelated loopback hits — a browser fetching `/favicon.ico` no longer counts as the callback, and can no longer overwrite one already captured. The server is closed in a `finally` block.
- **`videos --short-form --limit N` returned fewer than N videos.** The pagination loop stopped after collecting N *uploads* and only then dropped everything over `SHORT_FORM_MAX_SECONDS`, so on a mixed-format channel the caller silently got a short list — sometimes empty — while qualifying shorts sat on the next page. `--limit` now counts videos returned, not uploads inspected: it keeps paginating until N qualifying videos are collected or the uploads playlist is exhausted.
